...
@Component
@Transactional
public class FaceMatcherPersonFullDataService extends EntityService<FaceMatcherPersonFullData> implements FaceMatcherPersonFullDataDao {
...
//        context.setAttribute("sdk", sdk);
	...
	public SearchResultDTO search(byte[] data) throws Exception{
        ...
        if(afidList.size()==0){
        	...
        	List<FaceMatcherPersonFullData> list = findAll();
...
//        byte[] a = afidList.get(index);
        ...
        final ForkJoinPool pool = new ForkJoinPool(18);
...
//        byte[] a = afidList.get(index);
	...
	public FaceMatcherPersonFullData findByPropertyUnique(String property,Object value) throws Exception{
		List<FaceMatcherPersonFullData> a = findByProperty("afid", value);
		if(a.size()>1)
			return a.get(0);
		if(a.size()==1)
			return a.get(0);
...
Date t4 = new Date();
	...
	public SearchResultDTO compare(String id, byte[] data2) throws Exception {
		...
		FaceMatcherPersonFullData person =findById(new Long(id));
